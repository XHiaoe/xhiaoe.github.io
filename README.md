...
> **仓库是公开的，而"网站上没显示" ≠ "别人看不到"。**

| 位置 | 谁能看到 | 看得到什么 |
|---|---|---|
| GitHub 仓库（public） | **所有人** | 源码、`README.md`、配置文件、`published: false` 的文章、草稿、**删过的内容（Git 历史）**、Actions 构建日志 |
| 网站 xhiaoe.github.io | 所有人 | 只有**已发布**的文章和页面 |

构建是单向的：仓库 →（`hexo generate`）→ 网站。所以仓库里有的东西网站不一定展示，**但仓库里的东西全部是公开的**。

### 会被攻击打崩吗？基本不会

GitHub Pages 是**纯静态托管**，没有可攻击的服务器端：

| 常见攻击 | 是否有效 | 原因 |
|---|---|---|
| SQL 注入 / 拖库 | ❌ | 没有数据库 |
| 命令执行 / 上传 getshell | ❌ | 没有后端代码，服务器只负责发文件 |
| 越权 / 会话劫持 | ❌ | 没有登录系统 |
| 暴力破解后台 | ❌ | 没有后台 |
| DDoS 打崩 | ❌ | 流量走 GitHub 自己的 CDN，防护是企业级的 |
| HTTPS 证书过期 | ❌ | 自动签发和续期 |

真正要防的不是"被打崩"，而是下面这些**内容和习惯**上的问题。

### 想限制访问？先看你要哪种

GitHub Pages 免费方案**不支持访问控制**——站点一旦发布就是公开的。

| 需求 | 能否做到 | 做法 |
|---|---|---|
| 临时下线，谁都打不开 | ✅ | Settings → Pages → Source 改成 `None`。站点立刻 404，改回来就恢复 |
| 彻底关站 | ✅（有代价） | 仓库改 Private。免费账号下私有仓库**不能发布 Pages**，Actions 部署也会停 |
| 只让特定人看 | ❌ | 需要自定义域名 + Cloudflare Access，或换到支持密码保护的平台（Vercel / Netlify） |

> ⚠️ **"加个前端密码"是假保护。** HTML/CSS/JS 全在访客浏览器里，查看源码就能绕过；而且仓库公开，原文在 GitHub 上明文躺着。别信这类教程。

### 风险清单

#### ① 提交记录里的邮箱（已发生）

每条提交都会带上 `user.name` 和 `user.email`，公开仓库里谁都能看到，爬虫也会抓。修正方法：

1. GitHub → Settings → Emails → 勾选 **Keep my email addresses private**，会得到一个形如 `12345678+XHiaoe@users.noreply.github.com` 的地址
2. 本地换掉：

```bash
git config --global user.email "你的ID+XHiaoe@users.noreply.github.com"
git config --global --list | grep email    # 验证
```

以后的新提交就不暴露真实邮箱了。**但历史提交改不掉**（除非重写 Git 历史并 force push，对新手有风险，不建议现在做）。

#### ② 永远不要把 Token / 密钥提交进仓库

最致命的一条。**一旦提交，即使下一条 commit 删掉，Git 历史里还在**，GitHub 上有专门爬这个的机器人，通常几分钟内就会有人拿它来试。

真发生了怎么办：**第一步永远是去把那个 Token / 密钥作废（revoke）**，换新的比"删掉记录"重要得多；再考虑重写历史（可选，风险较高）。

日常习惯：密钥写进 `.env` 之类**已被 `.gitignore` 排除**的文件，绝不能出现在 `_config.yml`、文章正文或 `README.md` 里。

#### ③ `published: false` 不是"保密"

那只是"网站不显示"，**源码在 GitHub 上公开可见**。真正想保密的笔记：

- 放在**本地**不提交（`.gitignore` 排除，或提交时只 `git add` 指定文件）
- 或者另开一个 **private 仓库**存私密笔记

#### ④ Actions 日志也是公开的

仓库 public 时，构建日志谁都能翻。当前 workflow 只跑 `npm ci` + `hexo generate`，不打印任何敏感信息，没问题。以后如果在 workflow 里用 Secrets，注意别 `echo` 出来。

#### ⑤ 评论区才是真正有注入面的地方

- 当前 `comments.use` 为空，没有评论功能，这块风险为零
- 以后接 **Giscus**（推荐）：GitHub 托管 + iframe 隔离，安全责任在 GitHub 那边
- 要警惕**自建评论系统**（如 Valine 之类）：那才有真正的 XSS 和数据库注入面

#### ⑥ 写安全类文章时，别贴真实目标信息

这是做安全博客最容易踩、也最容易带来**真实麻烦**（而不只是技术风险）的坑：

- ❌ 公司内网 IP、客户域名、同事账号、未公开的漏洞细节
- ✅ 靶场环境（HTB、VulnHub、DVWA、自己搭的虚拟机）随便写
- ⚠️ 动手前先确认：这是靶场吗？授权范围包含它吗？

### 推送前自查（花三秒钟）

- [ ] 这次提交的文件里有没有密码 / Token / 密钥？
- [ ] 有没有公司或别人的隐私信息？
- [ ] 有没有还不想公开的草稿被误提交？
